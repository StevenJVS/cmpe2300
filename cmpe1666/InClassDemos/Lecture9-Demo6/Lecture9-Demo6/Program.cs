...
namespace Lecture9_Demo6
{
    internal class Program
    {
        ...
        {
            bool bIsPrime=true;//To check that our  value under test is a prime number
            ...
            if (objData is PrimeData)
            {
                ...
                for (int iNumber=Prime.Min; iNumber <= Prime.Max; iNumber++)
                {
                    ...
                    //We start from 2 and iterate up to iNumber-1, trying to find
                    //an exact divisor for the iNumber
                    //If no such divisor is found, the number is prime 
                    for (int iTry = 2; iTry <= iNumber - 1; iTry++)
                    { if (iNumber % iTry == 0)
                            bIsPrime = false;

                        //If the value is prime, display it
